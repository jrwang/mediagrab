...
class Movie(Item):
    ...
    def statistics(self, movie_details):
        ...
        def rating_to_float(rating):
            return float(rating) if rating != "N/A" else None
        ratings = [rating_to_float(m['imdbRating']) for m in movie_details]
        print "Average IMDB rating: {:.2f}".format(sum_with_sub(ratings)/len(ratings))
        print "-----------------------------"
...
class Book(Item):
    ...
    def statistics(self, book_details):
        ...
        print "Height of books if stacked (est.): {:.2f} cm".format(sum_with_sub(height_l))
        print "-----------------------------"
    ...
    def __repr__(self):
        ...
        try:
            ...
            return u"\"{}\" - {}{}".format(self.title, self.details['volumeInfo']['publisher'], star).encode("utf-8")
...
class Album(Item):
    ...
    def statistics(self, album_details):
        ...
        print "Top {} genres: {}".format(CAT_N, counter_top(top_genres, CAT_N))
        print "Total cost (iTunes): ${}".format(sum_with_sub([a['collectionPrice'] for a in album_details]))

        date_l = []
        for date in [a['releaseDate'] for a in album_details]:
            date_l.append(datetime.datetime.strptime(date, '%Y-%m-%dT%H:%M:%SZ'))
        ...
        total_ms = 0
        for a in album_details:
            total_ms += sum([t['trackTimeMillis'] for t in a['trackDetails']])
        #total_ms = sum([t['trackTimeMillis'] for t in a['trackDetails'] for a in album_details])
        total_t = datetime.timedelta(milliseconds=total_ms)
        print total_t
        days, hours, minutes = total_t.days, total_t.seconds/3600, (total_t.seconds/60) % 60
